...
export function PresupuestosTable() {
  ...
  const { data } = useSuspenseQuery(budgetsQueryOptions(filters));

  const pageCount = data.totalPages ?? Math.ceil(data.total / params.perPage);
  ...
  const emptyState = (
    <EmptyState
      ...
      description='Creá el primer presupuesto con el botón "Nuevo presupuesto".'
    />
  );
...
export function PresupuestosTableSkeleton() {
  return (
