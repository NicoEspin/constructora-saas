...
import { getProjectOperationalReport } from '@/features/reportes/api/service';
import {
...
export default async function OperationalProjectReportPrintPage({ params }: PageProps) {
  const { id } = await params;
  const [report, tenant, setting] = await Promise.all([
    getProjectOperationalReport(id),
    getCurrentTenant(),
