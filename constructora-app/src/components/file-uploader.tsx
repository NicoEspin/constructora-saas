    ...
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ file }) => {
          toast.error(`Se rechazó el archivo ${file.name}`);
        });
      }

      if (onUpload && updatedFiles.length > 0 && updatedFiles.length <= maxFiles) {
        const target = updatedFiles.length > 1 ? `${updatedFiles.length} archivos` : 'archivo';

        toast.promise(onUpload(updatedFiles), {
          loading: `Subiendo ${target}...`,
          success: () => {
            setFiles([]);
            return `${target} subido${updatedFiles.length > 1 ? 's' : ''}`;
          },
              ...
              <div className='flex flex-col items-center justify-center gap-4 sm:px-5'>
                ...
                <div className='flex flex-col gap-px'>
                  <p className='text-muted-foreground font-medium'>
                    Arrastrá archivos acá o hacé clic para seleccionarlos
                  </p>
                  <p className='text-muted-foreground/70 text-sm'>
                    ...
                    {maxFiles > 1
                      ...
                      : ` 1 archivo de hasta ${formatBytes(maxSize)}`}
                  </p>
                </div>
              </div>
            )}
...
function FileCard({ file, progress, onRemove }: FileCardProps) {
  const shouldRenderPreview = isPreviewableImage(file);
...
function isPreviewableImage(file: File): file is File & { preview: string } {
  return isFileWithPreview(file) && file.type.startsWith('image/');
}
