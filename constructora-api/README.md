...
Attachment storage via Cloudflare R2 also requires:
...
Attachment upload/access endpoints require these variables at runtime and will return a server error if any of them are missing.

If the browser uploads directly to R2 using presigned URLs, the bucket also needs a CORS rule that allows your frontend origin. For local development with `http://localhost:3000`, a minimal rule is:
...
Without bucket CORS, presigned uploads can work from server-side tools and still fail in the browser during the preflight request.
...
For applying existing migrations to an existing database without creating new migrations or running seed:
